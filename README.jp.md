...
Codex Workspace は、VS Code 上で散らばりがちな Codex の設定や運用情報を“ひとつに集約”し、迷わず管理できる拡張機能です。`config.toml` や `AGENTS.md` はもちろん、サブエージェント／Skills／Commands／Templates／MCP サーバー、会話履歴、Feature Flags、Hooks 診断、インストール済み Plugin の構成情報までをまとめて扱えます。「どこに何があるか」を探す時間を減らし、すぐに作業へ戻れます。
...
- `Codex Manager`、`Sub Agents`、`Skills`、`Commands`、`MCP Server`、`Templates`、`Codex Pet` の専用 Explore を提供
...
- 会話履歴、AGENTS Loading Chain、Trusted Directory、Feature Flags、Hooks、Plugins をまとめた `Codex Manager` を提供
- Plugin の表示と有効/無効の管理は、Skills / Sub Agents / MCP の Explore / Manager ではなく `Codex Manager` の `Plugins` タブに集約
...
- `Plugins`
  - ローカル plugin cache からインストール済み Plugin を一覧表示
  - manifest 由来の metadata、同梱 Skills / MCP / Apps / Agents、marketplace source 情報を表示
  - manifest と `config.toml` が操作可能な状態なら `[plugins."<id>"]` を通じて有効/無効を切り替え
...
`Codex Pet` は、アニメーションするペットを表示する専用の Webview Explore です。

- `~/.codex/pets/<petId>` からペットを読み込みます
- タイトルバーからペットを選択でき、選択済み `petId` を保持します
- Explore 内では、idle / drag / click / wait アニメーションを持つドラッグ可能なキャラクターを表示します
- 必要に応じて `codex app-server` と連携し、更新後に `5h` / `1w` の rate limit 吹き出しを表示します
...
| `Codex Workspace: Select Pet` | `Codex Pet Explore` で表示するペットを Quick Pick から選択します |
| `Codex Workspace: Connect Pet App Server` | 機能が有効な場合に `Codex Pet Explore` を `codex app-server` へ接続します |
| `Codex Workspace: Disconnect Pet App Server` | `Codex Pet Explore` の `codex app-server` 接続を切断します |
...
| `codex-workspace.pet.enabled` | boolean | `false` | Activity Bar コンテナ内で `Codex Pet Explore` ビューを有効にします。 |
| `codex-workspace.pet.appServer.enabled` | boolean | `false` | ペットビューの title bar に接続/切断コマンドを表示し、App Server の rate limit 連携を有効にします。 |
| `codex-workspace.pet.rateLimitRefreshMinutes` | number | `5` | `codex app-server` から rate limit を再取得する間隔です。単位は分で、`0` は自動更新無効です。 |
| `codex-workspace.pet.scale` | number | `1` | ペットの表示倍率です。対応範囲は `0.5` から `2.0` です。 |
| `codex-workspace.pet.selectedPetId` | string | `""` | 現在選択中のペット ID を保持します。 |
