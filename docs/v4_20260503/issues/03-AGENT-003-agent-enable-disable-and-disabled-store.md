...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [機能] Agentノードのコンテキストメニューから有効化/無効化を実行できる
  - [x] [機能] 無効化時に`config.toml`から`[agents.<agent>]`が削除される
  - [x] [機能] 無効化時に削除ブロック（コメント含む）が`agents-disabled.json`へ保存される
  - [x] [機能] 有効化時に退避ブロックが存在すればその内容で復元される
  - [x] [機能] 退避ブロックが無い場合は最小構成ブロックが追加される
  - [x] [UI/UX] 切替成功後に再起動が必要な旨の通知が表示される
  - [x] [テスト] 退避/復元と最小構成フォールバックのテストが追加される
