...
- [x] [機能] `~/.codex` か `config.toml` が欠落/読取不可/パース不可の場合、各ビューに `? Codex Workspace を開けません: <理由>` が1件のみ表示される
- [x] [状態/エラー] 利用不可状態では全コマンドが実行されず、ファイル操作が発生しない
- [x] [UI/UX] 対象未選択で操作した場合、通知メッセージが表示される
- [x] [テスト] 利用可否判定の主要ケースがユニットテストで検証できる
