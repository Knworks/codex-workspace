...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [機能] `agentFolder` 設定が追加され、既定値が空文字になっている
  - [x] [UI/UX] `agentFolder` 未設定時に Agent Explorer の同期ボタンが表示されない
  - [x] [機能] `agentFolder` 設定時に Agent Explorer の同期ボタンから同期処理を実行できる
  - [x] [機能] Agent同期で同名ファイルは最終更新日時が新しい側を正として上書きされる
  - [x] [機能] Agent同期で削除同期が行われ、判定メタが `codex-sync.json` で管理される
  - [x] [状態/エラー] 上書き中エラーは該当ファイルのみスキップされ、通知される
  - [x] [テスト] Agent同期の表示条件・同期処理・削除同期のテストが追加される
