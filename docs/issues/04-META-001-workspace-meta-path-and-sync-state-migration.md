...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [機能] 同期メタが`.codex/.codex-workspace/codex-sync.json`へ保存される
  - [x] [機能] 読み取り優先順が新保存先→旧保存先になっている
  - [x] [機能] 新が無く旧がある場合に旧から新へ原子的に移行される
  - [x] [機能] 移行成功時に旧`.codex/.codex-sync/state.json`が削除される
  - [x] [状態/エラー] 移行失敗時は旧ファイルが保持され、処理が中断される
  - [x] [機能] 同期対象除外パスが`.codex/.codex-workspace`に更新される
  - [x] [テスト] 正常移行/失敗時ロールバック/優先読取のテストが追加される
