...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [テスト] Agent Explorerの一覧/選択/アイコン切替がテストで検証できる
  - [x] [テスト] Agent追加フロー（名前/説明/テンプレート）と重複中断がテストで検証できる
  - [x] [テスト] Agent同期のボタン表示条件と相互同期処理がテストで検証できる
  - [x] [テスト] `config.toml`の`[agents.<agent>]`追記・非上書き通知がテストで検証できる
  - [x] [テスト] Agent無効化時の退避（コメント保持）と有効化時復元がテストで検証できる
  - [x] [テスト] 同期メタの新旧読取優先順と原子的移行がテストで検証できる
  - [x] [テスト] 移行失敗時に旧メタ保持・中断となることがテストで検証できる
  - [x] [テスト] `.codex/codex-templates`継続利用と自動移動/削除なしをテストで検証できる
