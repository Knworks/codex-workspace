...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [機能] Agent追加で名前・説明・テンプレート選択の順に入力できる
  - [x] [機能] 空ファイル選択時はテンプレート未適用で`.codex/agents/<agent>.toml`が作成される
  - [x] [機能] テンプレート選択時はテンプレート内容が新規Agentファイルへ反映される
  - [x] [状態/エラー] 同名Agentファイルが存在する場合は作成せず衝突通知される
  - [x] [機能] 作成成功時に`config.toml`へ`[agents.<agent>]`が自動追記される
  - [x] [状態/エラー] 既存`[agents.<agent>]`がある場合は上書きせず通知される
  - [x] [テスト] 追加/編集/削除フローと`config.toml`追記のテストが追加される
