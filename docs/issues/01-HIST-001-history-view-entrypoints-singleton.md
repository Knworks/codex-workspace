...
- AC：必要十分な項目数（最低3、目安5〜10、上限なし）
  - [x] [機能] Codex Core の履歴ボタン押下で履歴ビューがエディタ領域に表示される
  - [x] [機能] コマンドパレット実行で同一の履歴ビューが表示される
  - [x] [機能] 履歴ビュー表示中に再実行すると新規生成せず既存ビューが前面化される
  - [x] [UI/UX] 履歴ボタンは Codex Core の既存ボタン配置を崩さず表示される
  - [x] [テスト] 単一インスタンス制御とコマンド登録のテストが追加される
