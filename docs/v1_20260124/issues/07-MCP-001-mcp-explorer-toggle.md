...
- [x] [機能] MCP サーバーが `config.toml` の出現順で表示される
- [x] [機能] `enabled` がない場合はヘッダ直下に `enabled = false` が挿入される
- [x] [状態/エラー] `enabled = true # comment` の末尾コメントが保持される
- [x] [UI/UX] ON/OFF 状態がスイッチ風 UI とアイコンで視認できる
- [x] [UI/UX] トグル成功時に再起動が必要な旨の通知が表示される
- [x] [テスト] `enabled` パッチ処理の主要パターンがテストで検証できる
