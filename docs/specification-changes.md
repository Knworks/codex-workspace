...
- Sub Agents Exploreをサブエージェント専用に整理し、AGENTS Manager Viewを追加する。
- MCP Server Exploreは現行仕様を維持し、MCP Manager Viewを追加する。
- Commands ExploreはObsolete機能として扱い、現行仕様を維持する。
- Template Exploreは変更不要とし、現行仕様を維持する。
- Codex Core Exploreを整理し、Codex Managerを追加する。
- 既存の会話履歴画面は、Codex Managerの会話履歴タブへ移行する。
- Codex Managerには、会話履歴、AGENTS Loading Chain、信頼するディレクトリ、Feature Flags、Hooksをタブ表示する。
...
- ユーザーはCodex Managerの会話履歴タブで、現行の会話履歴を閲覧・検索・コピーできる。
...
- 新規ファイル作成時は、Skills ルート選択時のみ作成先の保存場所を選択できる。
- 作成先選択肢は以下とする。
  - Project Skills
  - Workspace Skills
  - User Skills
...
- 新規フォルダ作成時は、Skills ルート選択時のみ作成先の保存場所を選択できる。
- 作成先選択肢は以下とする。
  - Project Skills
  - Workspace Skills
  - User Skills
...
### 🧑‍💻 Sub Agents Explore / AGENTS Manager View

#### 🗂️ 対象範囲

Sub Agents ExploreおよびAGENTS Manager Viewでは、以下のみを対象とする。
...
Sub Agents ExploreおよびAGENTS Manager Viewは、Skillsと同じ考え方で以下の保存場所を対象とする。

| 種別 | 表示名 | 参照ルート | 優先度 |
| --- | --- | --- | --- |
| `project` | Project Agents | `project/.codex/agents` を優先し、存在しない場合は `project/.agents/agents` を読む | 1 |
...
#### 🧭 Sub Agents Exploreの表示

- Sub Agents Exploreは、サブエージェント定義ファイルを単一一覧として表示する。
- Sub Agents Exploreでは、ファイル名のみを表示する。
- 保存場所ごとに異なるアイコンを表示する。
- ツールチップには、保存場所種別とファイルパスを表示する。
- Sub Agents Exploreでは、ON/OFFトグルを提供しない。
- ON/OFF操作は、AGENTS Manager Viewへ移動する。
- AGENTS Manager Viewでトグル操作が行われた場合、Sub Agents Exploreの表示を更新する。
...
- 未選択時は、Project Agentsを初期候補にする。
- Project Agents を選んだ場合の新規作成先は、読込元が `project/.agents/agents` であっても `project/.codex/agents` とする。
- 作成先保存場所が存在しない場合は、自動作成する。
...
- Sub Agents Exploreの同期仕様は、現行仕様をそのまま維持する。
- 同期ボタンの有無、表示条件、同期元、同期先、同期方式は現行仕様から変更しない。
- 今回の変更では、Sub Agents Exploreの同期仕様は変更対象外とする。
- AGENTS Manager Viewには同期機能を追加しない。
- AGENTS Manager ViewでON/OFFトグルを操作した場合は、Sub Agents Exploreの表示を更新する。

#### 🔁 Refresh

- Refresh操作は、Sub Agents ExploreとAGENTS Manager Viewの双方を更新対象に含める。
- Sub Agents Exploreを再読み込みする。
- AGENTS Manager Viewが開いている場合、一覧を再取得して再描画する。
- AGENTS Manager Viewでトグル操作を行った場合も、Sub Agents Exploreを更新する。
...
- Sub Agents Exploreでは、サブエージェントのON/OFFトグルを提供しない。
...
- トグル成功後は、設定変更の反映にCodexの再起動が必要である旨を通知する。
- トグル操作後は、AGENTS Manager ViewとSub Agents Exploreの表示を更新する。
...
### 🔌 MCP Server Explore / MCP Manager View

#### 🔌 MCP Server Explore

MCP Server Exploreは、現行仕様を維持する。
...
- MCP Manager Viewで編集、追加、削除が行われた場合は、MCP Server Exploreを更新する。
...
- ON/OFFトグルの更新処理は、MCP Server Exploreの現行処理と同じものを使用する。
...
- 保存後、未保存状態を解除する。
- 保存後、MCP Server ExploreとMCP Manager Viewを更新する。
...
- 保存後は、MCP Server ExploreとMCP Manager Viewを更新する。
...
- 削除後は、MCP Server ExploreとMCP Manager Viewを更新する。
...
- ON/OFFトグルは即時保存する。
- ON/OFFトグルの処理はMCP Server Exploreの現行処理と同じものを使用する。
- `enabled`が未定義の場合はONとして扱う。
- OFF操作時に`enabled`が未定義の場合は、対象ブロックに`enabled = false`を追加する。
- `enabled`行が存在する場合は、`true` / `false`を切り替える。
- トグル後は、MCP Server ExploreとMCP Manager Viewを更新する。
- トグル後は、設定変更の反映にCodexの再起動が必要である旨を通知する。

### 💬 Commands Explore

- Commands Exploreは、Obsolete機能として扱う。
- Commands Exploreの現行仕様は維持する。
- Commands Exploreには、今回の変更では新機能を追加しない。
- Commands Explore用のManager Viewは追加しない。
- Commands Exploreは、Codex CLIまたはVS Code拡張の公式カスタムスラッシュコマンド管理機能としては扱わない。
- Commands Exploreで管理するPromptが、Codex CLIまたはVS Code拡張でカスタムスラッシュコマンドとして利用できることは保証しない。
- Commands Exploreの表示/非表示は、既存のユーザー設定に従う。
- Commands Exploreの同期仕様は、現行仕様を維持する。
...
### 🧭 Codex Core Explore / Codex Manager
...
既存の会話履歴ボタンは廃止し、Codex Managerを開くボタンへ置き換える。
...
- `config.toml`の解析に失敗している場合でも、`.codex`フォルダを開く操作、`AGENTS.md`を開く操作、`AGENTS.override.md`を開く操作、Codex Managerを開く操作は利用可能とする。
- `config.toml`の解析に依存するMCP一覧、信頼するディレクトリ一覧、Skills/AGENTS/MCPの設定更新操作は無効化する。
- Codex Managerの会話履歴タブとAGENTS Loading Chainタブは、`config.toml`の解析に失敗していても表示可能とする。
...
- Codex Managerは、Core関連の詳細画面をタブで表示する管理画面とする。
- Codex Managerは、WebviewPanelを利用してエディタ領域に表示する。
- Codex Managerは、単一インスタンスとする。
- 既に開いている場合は、新規作成せず前面表示する。
- Codex Managerは、タブ切り替え形式で複数のCore関連画面を表示する。
- Codex Managerの初期表示タブは、会話履歴タブとする。
- Codex Managerには、以下のタブを表示する。
  - 会話履歴
...
- 会話履歴タブは、Codex Managerの初期表示タブとする。
- 既存の会話履歴画面で提供している検索、一覧、詳細表示、Markdownプレビュー、コピー操作は維持する。
- 会話履歴データの取得元、解析方式、表示順、表示件数制限は現行仕様を維持する。
- 既存の会話履歴ボタンは廃止し、Codex Managerボタンへ置き換える。
- Codex Managerボタンを押下した場合、Codex Managerを開き、会話履歴タブを表示する。
- Codex Managerが既に開いている場合は、既存インスタンスを前面表示し、会話履歴タブへ切り替える。
...
- Skill Manager View、AGENTS Manager View、MCP Manager View、Codex Managerは、WebviewPanelを利用してエディタ領域に表示する。
...
- Codex Managerは、タブ切り替え形式でCore関連画面を表示する。
...
| AGENTS対象除外 | `AGENTS.md` / `AGENTS.override.md`がSub Agents Exploreに表示されない |
...
| MCP Server Explore維持 | MCP Server Exploreの現行ON/OFF仕様が維持される |
...
- Sub Agents Exploreの同期仕様は現行仕様を維持し、今回の変更対象外とする。
- Commands ExploreはObsolete機能として扱い、今回の変更では拡張しない。
...
今回の変更では、既存Exploreを壊さず、管理・診断・編集が必要な領域をManager ViewまたはCodex Managerへ分離する。

- Skillsは、複数保存場所対応とSkill Manager Viewを追加する。
- AGENTSは、Sub Agents Exploreとしてサブエージェント専用に整理し、AGENTS Manager Viewを追加する。
- MCPは、MCP Server Exploreの現行ON/OFF仕様を維持し、MCP Manager Viewを追加する。
- PROMPTSは、Commands ExploreとしてObsolete機能のまま現行維持する。
- Templateは、変更不要として現行維持する。
- Codex Coreは、Codex Managerを追加し、会話履歴、AGENTS Loading Chain、信頼するディレクトリ、Feature Flags、Hooksをタブで扱う。
