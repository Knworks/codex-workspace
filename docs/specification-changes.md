...
- ユーザーはAGENTS Loading Chainタブで、AGENTS系ファイルの読み込み順、採用状態、スキップ理由を確認できる。
...
- AGENTS Loading Chainタブは、AGENTS系ファイルの読み込み状態を視覚的に把握できるUIとする。
- 左ペインは、読み込み順を上から下へ表現する縦方向のチェーン表示とする。
- 各ノードには、ステップ番号、保存場所を示すアイコン、ファイル名、パス、状態バッジを表示する。
- 状態の識別は、色だけでなく、アイコン、バッジ、文字、トーン差、線種の組み合わせで表現する。
- Skipped状態のノードは、暗転表示とし、読み込み対象外であることを視覚的に識別できるようにする。
- 各ノードには、状態理由を短い補足テキストまたはツールチップで表示する。
- ヘッダには、基準ワークスペースルートと、候補数、有効数、Skipped数などの集計情報を表示する。

##### 🧬 AGENTS Loading Chainのノード状態

AGENTS Loading Chainでは、Skippedファイルも表示する。

状態は、以下の4種類とする。

| 状態 | 意味 |
| --- | --- |
| `Active` | Codexの読み込み対象として採用される |
| `Skipped` | 候補ではあるが、優先順位の関係で読み込まれない |
| `Missing` | 候補として探索したが存在しない |
| `Error` | 存在するが読み取りに失敗した |
...
- `Skipped`、`Missing`、`Error`のノードには、理由を短い補足テキストまたはツールチップで表示する。
...
- `Missing`は、初期リリースでは`project_doc_fallback_filenames`に指定された候補ファイルが存在しない場合のみ表示する。

##### 📄 AGENTS Loading Chainの右ペイン

右ペインには、左ペインで選択したノードの状態理由、詳細情報、本文プレビューを表示する。

- 右ペインには、ファイル名、状態、種別、読み込み順、絶対パス、状態理由を表示する。
- `Active`のノードには、読み込み順と採用理由を表示する。
- `Skipped`のノードには、読み込み対象外となった理由と、優先されたファイルを表示する。
- `Missing`のノードには、候補ファイルが存在しない理由を表示する。
- `Error`のノードには、読み取り失敗理由とエラーメッセージを表示する。
...
- AGENTS Loading Chainは、グラフィカルに状態が分かる縦型チェーン表示とする。
- AGENTS Loading Chainの状態識別は、色だけに依存せず、アイコン、バッジ、文字、トーン差、線種を組み合わせる。
...
| AGENTS Loading Chain | ワークスペースルート基準でチェーンが表示される |
| Loading Chain状態表示 | Active / Skipped / Missing / Errorが表示される |
| Loading Chain種別表示 | Global / Project / Standard / Override / Fallbackが表示される |
| Loading Chain右ペイン | 状態理由、詳細情報、本文プレビューが表示される |
