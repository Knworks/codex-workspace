...
* 目標名が存在する場合：ユーザーに選択肢を提示する（その場で確認して選べる）

  * 選択肢

    * `上書きする`：既存ファイルを削除して置換（確認ダイアログ必須）
    * `別名にする（_1）`：`_1`, `_2`… の連番で空き名に回避してリネーム
    * `キャンセル`：何もしない
  * `上書きする` を選んだ場合、事前確認ダイアログの文言例：
    `同名ファイルを上書きします。既存ファイルは削除されますがよろしいですか？`
